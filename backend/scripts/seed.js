...
const seed = async () => {
  ...
  await query('DELETE FROM appointments WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM schedules WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM prescriptions WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM notifications WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM events WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM doctors WHERE user_id IN ($1, $2, $3)', [DEMO_ADMIN_ID, DEMO_DOCTOR_ID, DEMO_PATIENT_ID]);
  await query('DELETE FROM patients WHERE user_id IN ($1, $2, $3)', [DEMO_ADMIN_ID, DEMO_DOCTOR_ID, DEMO_PATIENT_ID]);
  await query('DELETE FROM users WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  await query('DELETE FROM hospitals WHERE hospital_id = $1', [DEMO_HOSPITAL_ID]);
  ...
  await query(
    ...
    [DEMO_HOSPITAL_UUID, DEMO_HOSPITAL_ID, 'AayuCare Demo Hospital', '123 Healthcare Blvd', '+91-9876543210', 'hospital@aayucare.com', true]
  );
  ...
  await query(
    ...
    [DEMO_DOCTOR_ID, DEMO_DOCTOR_ID, 'Cardiologist', 'MBBS, MD (Cardiology)', 12, 800.00, 'Experienced cardiologist specializing in preventive cardiology.']
  );
  ...
  for (const day of days) {
    await query(
      ...
      [DEMO_DOCTOR_ID, DEMO_HOSPITAL_ID, day, '09:00:00', '17:00:00', 15, true, 20]
    );
  }
  ...
  await query(
    `INSERT INTO patients (id, user_id, date_of_birth, gender, blood_group, address)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [DEMO_PATIENT_ID, DEMO_PATIENT_ID, '1990-05-15', 'male', 'O+', '456 Patient Lane']
  );

...
if (!process.env.DATABASE_URL) {
  ...
  if (missingPg.length > 0) {
    missingVars.push('DATABASE_URL (or POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB)');
...
if (process.env.JWT_SECRET && process.env.JWT_SECRET.length < MIN_JWT_SECRET_LENGTH) {
  if (process.env.NODE_ENV === "production") {
    logger.error(`❌ FATAL: JWT_SECRET is too weak (${process.env.JWT_SECRET.length} chars). Must be at least ${MIN_JWT_SECRET_LENGTH} in production.`);
    process.exit(1);
  } else {
    logger.warn(`⚠️  JWT_SECRET is shorter than ${MIN_JWT_SECRET_LENGTH} characters. This will crash in production.`);
  }
}

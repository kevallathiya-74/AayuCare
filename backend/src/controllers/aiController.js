...
exports.getDietRecommendations = async (req, res) => {
    try {
        const { age, weight, height, conditions = [], allergies = [], goal = 'maintain' } = req.body;

        const dietPlan = generateDietPlan(age, weight, height, conditions, allergies, goal);
...
exports.getExerciseRecommendations = async (req, res) => {
    try {
        const { age, fitness = 'moderate', conditions = [], goal = 'fitness' } = req.body;
...
function analyzeSymptomsPattern(symptoms, severity) {
    ...
    if (hasFever && hasFlu) {
        conditions.push({ name: 'Viral Fever', probability: 75, severity: 'moderate' });
    }
    if (hasFever && hasCold && hasFlu) {
        conditions.push({ name: 'Seasonal Flu', probability: 70, severity: 'moderate' });
    }

    // Default if no matches
    if (conditions.length === 0) {
        conditions.push({ name: 'General Illness', probability: 50, severity: severity });
    }
...
function generateRecommendations(symptoms, severity) {
    return [
        ...
        severity === 'severe' ? 'Consult a doctor immediately' : 'Consult a doctor if symptoms worsen',
...
function generateRiskRecommendations(riskLevel, riskFactors) {
    ...
    if (riskFactors.includes('Elevated blood sugar')) {
        recommendations.push('Monitor blood sugar levels and consult endocrinologist');
    }
...
function generateDietPlan(age, weight, height, conditions, allergies, goal) {
    return {
        ...
        snacks: ['Fresh seasonal fruits', 'Nuts (almonds, walnuts)', 'Vegetable sticks with hummus'],
        nutritionGoals: {
        ...
        avoidFoods: ['Processed foods', 'Sugary beverages', 'Excessive salt', ...allergies],
    };
}
...
async function analyzeRecordWithAI(record) {
    ...
    return {
        ...
        preventiveMeasures: ['Regular health monitoring', 'Follow prescribed medications'],
    };
}

...
class NotificationService {
  ...
  async sendAppointmentReminder(appointment, patient, doctor) {
    try {
      ...
      logger.info(
        `[Notification] Appointment reminder sent to ${patient.name}`,
      );
    } catch (error) {
      logger.error(
  ...
  async sendPrescriptionNotification(prescription, patient, doctor) {
    try {
      ...
      logger.info(
        `[Notification] Prescription notification sent to ${patient.name}`,
      );
    } catch (error) {
      logger.error(
        "[Notification] Prescription notification failed:",
        error.message,
      );
    }
  }
  ...
  async sendPushNotification(notification) {
    try {
      ...
      if (!user || !user.expo_push_token) {
        logger.info(
          `[Notification] Skipped push for ${notification.userId}: No Expo token`,
        );
        return false;
      }

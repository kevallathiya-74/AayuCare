...
class TwilioService {
  ...
  async sendSMS(to, body) {
    try {
      ...
      logger.info(
        `[Twilio] SMS sent successfully to ${to} - SID: ${message.sid}`,
      );
      return {
        success: true,
  ...
  async sendOTP(phoneNumber) {
    try {
      ...
      logger.info(
        `[Twilio] OTP sent to ${phoneNumber} - Status: ${verification.status}`,
      );
      return {
        success: true,
  ...
  async verifyOTP(phoneNumber, code) {
    try {
      ...
      logger.info(
        `[Twilio] OTP verification for ${phoneNumber}: ${isValid ? "SUCCESS" : "FAILED"}`,
      );
...
Please consult your doctor if you have any concerns.
  ...
  async sendBulkSMS(recipients, message) {
    try {
      if (!this.checkConfiguration()) {
        logger.info(
          `[Twilio] Bulk SMS would be sent to ${recipients.length} recipients`,
        );
        return { success: false, message: "SMS service not configured" };
      }
      ...
      logger.info(
        `[Twilio] Bulk SMS completed - Success: ${successful}, Failed: ${failed}`,
      );

...
const SettingsScreen = ({ navigation }) => {
    ...
    (async () => {
      try {
        ...
        if (stored) {
          ...
          if (typeof prefs.appointmentReminders === "boolean")
            setAppointmentReminders(prefs.appointmentReminders);
          if (typeof prefs.medicationReminders === "boolean")
            setMedicationReminders(prefs.medicationReminders);
          if (typeof prefs.healthTips === "boolean")
            setHealthTips(prefs.healthTips);
        }
      } catch (err) {
    ...
    try {
      ...
      const currentPrefs = {
        ...
        appointmentReminders:
          settingName === "appointmentReminders" ? value : appointmentReminders,
        medicationReminders:
          settingName === "medicationReminders" ? value : medicationReminders,
  ...
  const aboutSettings = [
    ...
    },
    {
      ...
      rightIcon: { name: "chevron-forward" },
      onPress: () =>
        Alert.alert("Help & Support", "For assistance, contact us:", [
          {
            text: "Email Support",
            onPress: () => openURL("mailto:support@aayucare.in"),
          },
          {
            text: "WhatsApp",
        ...
        >
          <View style={styles.headerRow}>
            <TouchableOpacity
              ...
              onPress={() =>
                handleSmartBack(navigation, getSettingsBackFallback())
              }

...
export default function useDoctorForm({ mode, doctor, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let errorMessage = mode === "add" ? "Failed to add doctor. Please try again." : "Failed to update doctor profile. Please try again.";
      if (typeof error === "string") errorMessage = error;
      else if (error.response?.data?.message) errorMessage = error.response.data.message;
      else if (error.message) errorMessage = error.message;
      if (errorMessage.includes("already exists")) {
        if (errorMessage.includes("email")) errorMessage = "This email is already registered. Please use a different email.";
        else if (errorMessage.includes("phone")) errorMessage = "This phone number is already registered. Please use a different number.";
        else errorMessage = "A doctor with these details already exists. Please check email and phone number.";
      }
